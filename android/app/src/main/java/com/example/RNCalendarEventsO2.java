...
public class RNCalendarEventsO2 extends ReactContextBaseJavaModule implements PermissionListener {
  ...
  public RNCalendarEventsO2(ReactApplicationContext reactContext) {
    super(reactContext);
  ...
  private int addEvent(String title, ReadableMap details, ReadableMap options) throws ParseException {
    ...
    eventValues.put(CalendarContract.Events.CUSTOM_APP_PACKAGE, reactContext.getPackageName());
    eventValues.put(CalendarContract.Events.CUSTOM_APP_URI, "example://gizmos");
//    eventValues.put(CalendarContract.Events.CUSTOM_APP_URI, "https://www.example.com");
  ...
  private void createAttendeesForEvent(ContentResolver resolver, int eventID, ReadableArray attendees) {
    ...
    for (int i = 0; i < attendees.size(); i++) {
      ...
      if (type == ReadableType.String) {
        ContentValues values = new ContentValues();
        values.put(CalendarContract.Attendees.ATTENDEE_EMAIL, attendee.getString("email"));
        values.put(CalendarContract.Attendees.ATTENDEE_RELATIONSHIP, CalendarContract.Attendees.RELATIONSHIP_ATTENDEE);
        values.put(CalendarContract.Attendees.ATTENDEE_TYPE, CalendarContract.Attendees.TYPE_OPTIONAL); // new
        values.put(CalendarContract.Attendees.EVENT_ID, eventID);
        if (fNameType == ReadableType.String) {
          values.put(CalendarContract.Attendees.ATTENDEE_NAME, attendee.getString("name"));
        }
